...
# Run
> python -m tests.rlhf.sac --debug-log=True
"""
import torch
...
class ACAgent():
    ...
    # run one episode
    def play(self, env, max_steps=10000, render=False):
        s, _ = env.reset()
        s = s.astype('float32')
        r = 0  # reward
        logs = {"video_frames": [],
...
if __name__ == '__main__':
    ...
    parser.add_argument('--no-wandb', dest='wandb', default=True, action='store_false', help='Disable WandB logging')
    parser.add_argument('--debug-log', default=False, action='store_true', help='Enable debug loggings')
    parser.add_argument('--project', type=str, default='RLRG_AC', help='WandB project name [str]')
    ...
    if config.wandb:
        import wandb
        wandb.init(project=config.project, config=config_dict)
    ...
    if config.wandb and config.debug_log:
        wandb.watch(agent.valuenet, log="all")
        wandb.watch(agent.policynet, log="all")
    prev_policynet_dict = {}
    prev_valuenet_dict = {}
    render_enable = False
    R = []
    ...
    for n in range(EPISODES_TO_RUN):
        ...
        R.append(r)
        if config.wandb:
            if len(logs["video_frames"])!=0:
                frames = np.stack(logs["video_frames"], axis=0).transpose(0,3,1,2)
                fig, ax = plt.subplots(3,1,sharex=True)
                ax[0].plot(np.stack(logs["observations"]))
                ax[0].set_ylabel("obs.")
                ax[1].plot(np.stack(logs["actions"]))
                ax[1].set_ylabel("actions")
                ax[2].plot(np.stack(logs["rewards"]))
                ax[2].set_ylabel("rewards")
                wandb.log({"video": wandb.Video(frames, fps=40),
                           "obs_act_rew_plot": fig}, commit=False)
            if config.debug_log and (n%10==0):
                policynet_update_rate, prev_policynet_dict = get_update_rate(prev_policynet_dict, agent.policynet.state_dict())
                valuenet_update_rate, prev_valuenet_dict = get_update_rate(prev_valuenet_dict, agent.valuenet.state_dict())
                wandb.log({"poclicynet_update_rate": policynet_update_rate,
                           "valuenet_update_rate": valuenet_update_rate}, commit=False)
            wandb.log({"Reward": r, "MA100 reward": np.mean(R[-100:])})
        if (n % 10 == 0):
