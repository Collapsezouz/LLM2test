...
## v1.2
前置模型: /nas/tmp/phbsxgpt/llama_plugin_v1.0
数据集: /nas/dataset/llm/phbs_llm/plugin_instruct/plugin_v1.2.jsonl
...
export deepspeed_config=llm_model/configs/deepspeed.json
export data_path=./logs/data/plugin_instruct/plugin_v1_2/instruction_data.jsonl
export output_dir=./logs/llama_plugin_v1.2
...
deepspeed llm_model/alpaca/train.py \
    ...
    --model_max_length=2048 \
    --gradient_checkpointing=True \
    --instruct_version=2 > ./logs/ft_log/llama_plugin_v1.2.log 2>&1

## 训练完成后移动目录
清理状态文件夹: 
cd ./logs/llama_plugin_v1.2
find . -name global_step* | grep './checkpoint-' | xargs -i rm {} -rf
cd ../..
复制到nas目录: cp -r ./logs/llama_plugin_v1.2 /nas/tmp/phbsxgpt/
复制完成后, 删除原目录: rm ./logs/llama_plugin_v1.2 -rf
```

## v1.3
前置模型: /home/app/expert-cpt/logs/llama_oa_v0.1
数据集: /nas/dataset/llm/phbs_llm/plugin_instruct/plugin_v1_3_chat.jsonl


使用deepspeed库的训练过程:
```
export model_path=/home/app/expert-cpt/logs/llama_oa_v0.1
export CUDA_VISIBLE_DEVICES=4,5,6,7

export deepspeed_config=llm_model/configs/deepspeed.json
export data_path=/nas/dataset/llm/phbs_llm/plugin_instruct/plugin_v1_3_chat.jsonl
export output_dir=./logs/llama_plugin_v1.3
...
deepspeed llm_model/alpaca/train.py \
    ...
    --model_max_length=2048 \
    --gradient_checkpointing=True \
    --instruct_version=2 > ./logs/ft_log/llama_plugin_v1.3.log 2>&1
